...
FROM (
    SELECT winner.name AS team_name
    FROM games
    ...
    SELECT opponent.name AS team_name
    FROM games
...
echo $($PSQL "SELECT name FROM games FULL JOIN teams ON games.winner_id=teams.team_id WHERE winner_id=team_id GROUP BY name ORDER BY name;")
